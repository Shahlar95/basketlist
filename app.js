...
function basketproperty(){
  ...
  arr1.forEach(e=>{
    ...
    <span>${e.title}</span>
    <br>
    <span>discount price:${e.price} AZN</span>
    <br>
    <span>price:${e.oldprice} AZN</span>
    <br>
    <span onclick = 'basketDelete(${e.id})'>Delete film:[X]</span>
    <br>
    <span> Count: ${e.count -1+1}</span>
    <hr>
...
function sumPrice(){
    ...
    basketproperty().price-basketDelete(x).price;
    

}


function baketSumPrice() {

    let arr = getLocal("basket");
    
    arr.forEach(e=>{
        arr.innerHTML=`
        <span>Sum price:${sumPrice}</span>
        <br>
...
function basketDelete(x) {
    let arr2 = getLocal("basket");
    let wantDel = arr2.find(e=>e.id === x);

    if (wantDel.count === 1) {
        let arr3 = arr2.filter(e=>e.id !==x);
        setOne("basket",arr3);
    }
    else{
        let arr4 = arr2.map(e=>{
